...
| **Adaptive** | Pass-1 max (same Fixed field, default **40**), max rounds (default **8**), absolute cap (default **4096**) | Full progressive pass at pass-1 max, then only re-tests interior border pixels (4-connected to an escaped neighbor). At each doubled limit, keeps retesting newly exposed borders until a pass finds no new escapes, then doubles again (or stops if that first pass found nothing). Intermediate fills are painted on screen. After refine finishes, the highest limit that found new escapes is **carried** into the next Adaptive pass-1 (e.g. after zoom), so you do not restart from the Fixed max. Tip: lower pass-1 (e.g. 10–20) makes the first-frame border growth easier to see. |
