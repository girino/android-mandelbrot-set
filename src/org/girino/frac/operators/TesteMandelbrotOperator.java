 ...
 *
 */
public class TesteMandelbrotOperator implements FractalOperator {
	...
	public int apply(double x, double y, int maxiter) {
	    ...
	    for(j = 0; (j < maxiter) && (Complex.modulus(Z) <= omega);j++) {
	    	...
	    	//System.err.println(Complex.inverse(Z));
	    	Complex Zn = Complex.add(Complex.square(Complex.conjugate(Z)), C);
	    	Z = Zn;
	    }
